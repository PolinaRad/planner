...
?><a class="clickMe" href="logout.php">Logout</a>
    ...
    <form name="assignment" onSubmit="return formValidationAs();" action="change_assignment.php" method="POST">
 ...
 $sql_as = "SELECT `as_name`, `sub_id`, `as_percent`, `as_mark`, `as_max_mark`, `as_due_date`, `as_cur_date`, `user_id`, `as_desc` FROM `assignment` where `user_id` = '$user_id' and `as_id` = '$as_id'";
       
    $result = $mysqli->query($sql_as);
 ...
 $as_name=$row["as_name"];
       ...
       $as_percent=$row["as_percent"];  
        $as_mark=$row["as_mark"];
        $as_max_mark=$row["as_max_mark"];
        $as_due_date=$row["as_due_date"];
        $as_cur_date=$row["as_cur_date"];
        $as_desc=$row["as_desc"];
    ...
    ?>
        <TABLE>
            ...
            <TR> 
                ...
                <TD><INPUT type="text" id="as_name" NAME="as_name" SIZE=20 placeholder="name" value ="<?php echo $as_name; ?>" /></TD>
            </TR>
            
            <TR>
                <TD><label for="as_mark">Your score:</label></TD>
                <TD><INPUT type="number" id="as_mark" NAME="as_mark" SIZE=5  step="any" value =" " /></TD>
            </TR>

            <TR>
                <TD><label for="as_percent">Percentage:</label></TD>
                <TD><INPUT type="number" id="as_percent" NAME="as_percent" SIZE=5 placeholder="80" step="any" value =" "  /></TD>
            </TR>

            <TR>
                <TD><label for="as_max_mark">Maximum score:</label></TD>
                <TD><INPUT type="number" id="as_max_mark" NAME="as_max_mark" SIZE=5 placeholder="90.0" step="any" value ="" /></TD>
            </TR>
            <TR>
            ...
            <TR>
                ...
                <TD><textarea rows="10" cols="50" id="as_desc" name="as_desc" value =" " ></textarea></TD>
            </TR>
<TR>
...
if (isset($_POST['submit'])) {
    ...
    $as_name_ = val_input($_POST['as_name']);
  ...
  $sql = "SELECT `user_id` FROM `assignment` where `user_id` = '$user_id' and `sub_id` = '$sub_id' and `as_id`!= '$as_id' and `as_name` = '$as_name'";
...
if ($result_s->num_rows > 0) {
    ...
    alert("This assignment is already exist. Please change the name.");
      ...
      if(isset($_POST['as_name'])) 
     {
         ...
         $as_name = val_input_num($_POST['as_name']);
         $mysqli->query(" UPDATE `assignment` SET `as_name` = '$as_name' WHERE `assignment`.`as_id` = '$as_id'");
          
     }
       if(isset($_POST['as_cur_date'])) 
     {
        
         $as_cur_date = val_input_num($_POST['as_cur_date']);
         $mysqli->query(" UPDATE `assignment` SET `as_cur_date` = '$as_cur_date' WHERE `assignment`.`as_id` = '$as_id'");
          
     }
        if(isset($_POST['as_due_date'])) 
     {
        
         $as_due_date = val_input_num($_POST['as_due_date']);
         $mysqli->query(" UPDATE `assignment` SET `as_due_date` = '$as_due_date' WHERE `assignment`.`as_id` = '$as_id'");
          
     }
        if(isset($_POST['as_desc'])) 
     {
        
         $as_desc = val_input_num($_POST['as_desc']);
         $mysqli->query(" UPDATE `assignment` SET `as_desc` = '$as_desc' WHERE `assignment`.`as_id` = '$as_id'");
          
     }
      if(isset($_POST['sub_id'])) 
     {
        
         $sub_id = val_input_num($_POST['sub_id']);
         $mysqli->query(" UPDATE `assignment` SET `sub_id` = '$sub_id' WHERE `assignment`.`as_id` = '$as_id'");
     ...
     if(isset($_POST['as_mark'])) 
     {
         ...
         $mysqli->query(" UPDATE `assignment` SET `as_mark` = '$as_mark' WHERE `assignment`.`as_id` = '$as_id'");
          
     }
     if (isset($_POST['as_max_mark'])) 
     {  $as_max_mark = val_input_num($_POST['as_max_mark']);
      $mysqli->query(" UPDATE `assignment` SET `as_max_mark` = '$as_max_mark' WHERE ``assignment`.`as_id` = '$as_id'");
      }
   if (isset($_POST['as_percent']))
    {
           $as_percent = val_input_num($_POST['as_percent']);
         $mysqli->query(" UPDATE `assignment` SET `as_percent` = '$as_percent' WHERE `assignment`.`as_id` = '$as_id'");
    }
